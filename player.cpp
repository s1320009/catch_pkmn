...
//Player::Player() {
//	gameObject->position = { GetScreenWidth() / 2.0f, GetScreenHeight() / 2.0f };	//float型にするために2.0fとする
...
//
//			// 🛡️ 未来の座標を計算して画面外チェックを行う
//			Vector2 futurePos = this->gameObject->position;
//			futurePos.x += this->speed.x;
//			futurePos.y += this->speed.y;
//
//			// X方向のチェック：はみ出る場合は速度をゼロにして壁にピタッと止める（または跳ね返らせる）
//			if (futurePos.x + halfWidth < GetScreenWidth() && futurePos.x - halfWidth > 0) {
...
//			}
//			else {
//				this->speed.x = 0.0f; // 壁にぶつかったら横方向の勢いを止める
//			}
//
//			// Y方向のチェック
//			if (futurePos.y + halfHeight < GetScreenHeight() && futurePos.y - halfHeight > 0) {
...
//			}
//			else {
//				this->speed.y = 0.0f; // 壁にぶつかったら縦方向の勢いを止める
//			}
//
//			// プレイヤーのライフが0以下になった場合、状態を死んでいる状態に変更
//			if (this->life <= 0) {
...
//		case PLAYER_STATE_FINE: {
//			if (!isInvincible){
//				DrawRectangleV(gameObject->position, gameObject->scale, color);
//			}
//			break;
//		}
//		case PLAYER_STATE_DEAD: {
